...
public class BGARefreshLayout extends LinearLayout {
    ...
    private void initWholeHeaderView() {
        ...
        mWholeHeaderView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams
                .WRAP_CONTENT));
    ...
    public void onFinishInflate() {
        ...
        if (mContentView instanceof AbsListView) {
            ...
            mNestedScrollView = (NestedScrollView) mContentView;
        } else {
    ...
    public void setCustomHeaderView(View customHeaderView, boolean scrollable) {
        ...
        if (mCustomHeaderView != null) {
            mCustomHeaderView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT,
                    LayoutParams.WRAP_CONTENT));
    ...
    private void setAbsListViewOnScrollListener() {
        if (mAbsListView != null) {
            try {
                ...
                // mAbsListView.getClass().getDeclaredField("mOnScrollListener")
                // 获取不到mOnScrollListener，必须通过AbsListView.class.getDeclaredField
                // ("mOnScrollListener")获取
                Field field = AbsListView.class.getDeclaredField("mOnScrollListener");
                field.setAccessible(true);
                // 开发者自定义的滚动监听器
                final AbsListView.OnScrollListener onScrollListener = (AbsListView
                        .OnScrollListener) field.get(mAbsListView);
                mAbsListView.setOnScrollListener(new AbsListView.OnScrollListener() {
                    ...
                    public void onScrollStateChanged(AbsListView absListView, int scrollState) {
                        if ((scrollState == SCROLL_STATE_IDLE || scrollState ==
                                SCROLL_STATE_FLING) && shouldHandleAbsListViewLoadingMore
                                (mAbsListView)) {
                            beginLoadingMore();
                        }
    ...
    public boolean shouldHandleRecyclerViewLoadingMore(RecyclerView recyclerView) {
        if (mIsLoadingMore || mCurrentRefreshStatus == RefreshStatus.REFRESHING ||
                mLoadMoreFooterView == null || mDelegate == null || recyclerView.getAdapter() ==
                null || recyclerView.getAdapter().getItemCount() == 0) {
            return false;
        }
    ...
    private boolean shouldHandleLoadingMore() {
        ...
        if (BGARefreshScrollingUtil.isNestedScrollView(mNestedScrollView)) {
            return true;
        }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (!mIsLoadingMore && (mCurrentRefreshStatus != RefreshStatus.REFRESHING)) {
                    ...
                    int interceptTouchMoveDistanceY = (int) (event.getRawY() -
                            mInterceptTouchDownY);
                    // 可以没有上拉加载更多，但是必须有下拉刷新，否则就不拦截事件
                    if (Math.abs(event.getRawX() - mInterceptTouchDownX) < Math.abs
                            (interceptTouchMoveDistanceY) && mRefreshHeaderView != null) {
                        if ((interceptTouchMoveDistanceY > mTouchSlop && shouldHandleRefresh())
                                || (interceptTouchMoveDistanceY < -mTouchSlop &&
                                shouldHandleLoadingMore()) || (interceptTouchMoveDistanceY <
                                -mTouchSlop && !isWholeHeaderViewCompleteInvisible()) ||
                                (interceptTouchMoveDistanceY > mTouchSlop &&
    ...
    private boolean shouldHandleRefresh() {
        if (!mPullDownRefreshEnable || mIsLoadingMore || mCurrentRefreshStatus == RefreshStatus
                .REFRESHING || mRefreshHeaderView == null || mDelegate == null) {
            return false;
        }
    ...
    private boolean shouldInterceptToMoveCustomHeaderViewDown() {
        return isContentViewToTop() && mCustomHeaderView != null && mIsCustomHeaderViewScrollable
    ...
    private boolean handleActionMove(MotionEvent event) {
        ...
        if (mCustomHeaderView != null && mIsCustomHeaderViewScrollable) {
            ...
            if ((mPullDownRefreshEnable && !isWholeHeaderViewCompleteInvisible()) ||
                ...
                if (paddingTop < mMinWholeHeaderViewPaddingTop - mCustomHeaderView
                        .getMeasuredHeight()) {
                    paddingTop = mMinWholeHeaderViewPaddingTop - mCustomHeaderView
                            .getMeasuredHeight();
                }
                mWholeHeaderView.setPadding(0, paddingTop, 0, 0);
    ...
    private boolean handleActionUpOrCancel(MotionEvent event) {
        ...
        if ((mCustomHeaderView == null || (mCustomHeaderView != null &&
                !mIsCustomHeaderViewScrollable)) && mWholeHeaderView.getPaddingTop() !=
                mMinWholeHeaderViewPaddingTop) {
            isReturnTrue = true;
        }

        if (mCurrentRefreshStatus == RefreshStatus.PULL_DOWN || mCurrentRefreshStatus ==
                RefreshStatus.IDLE) {
            // 处于下拉刷新状态，松手时隐藏下拉刷新控件
            if (mCustomHeaderView == null || (mCustomHeaderView != null && mWholeHeaderView
                    .getPaddingTop() < 0 && mWholeHeaderView.getPaddingTop() >
                    mMinWholeHeaderViewPaddingTop)) {
    ...
    public void beginLoadingMore() {
        ...
        if (!mIsLoadingMore && mLoadMoreFooterView != null && mDelegate != null && mDelegate
                .onBGARefreshLayoutBeginLoadingMore(this)) {
            mIsLoadingMore = true;
    ...
    public interface BGARefreshLayoutDelegate {
         ...
         * 开始刷新
         */
        void onBGARefreshLayoutBeginRefreshing(BGARefreshLayout
                                                       refreshLayout);
